...
async function seed() {
	...
	dbResult.map(async (item) => {
		...
		const randomFee = insertFeesSchema.parse({
			...
			amount: String(Math.floor(Math.random() * (200 - 100 + 1)) + 100),
			category: String(Math.random() < 0.5 ? 'application' : 'processing'),
		});

		await database.insert(fees).values(randomFee);
	});
